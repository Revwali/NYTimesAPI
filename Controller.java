...
//	@Autowired
	...
	public News sayHi() throws JsonMappingException, JsonProcessingException, ParseException {
		...
		res = s.getBody();
		  ...
		  News news = new News(); Results[] results = new Results[3]; results[0] = new
		  Results(); results[0].setTitle(res.get("results").get(0).get("title"));
		//  news.setResults(results);
		  news.setSection(res.get("results").get(0).get("section"));
		  news.setTitle(res.get("results").get(0).get("title")); return news;
